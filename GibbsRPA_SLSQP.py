...
class GibbsSolver():    
    ...
    def Obj(self,x,RPA,Cs):
        ...
        return np.sum(np.array(f)**2)

    def GetErr(self,x,RPA,Cs):
        '''x: volume fraction and concentration of EFFECTIVE species
        x = [fI,CIpair1,CIpair2,CIpair3,...CIn]
        RPA class with defined interactions'''
    ...
    def Run(self):  
        ...
        for i in range(self.NeffSpecies):
            cons.append({'type': 'ineq', 'fun' : lambda x: self.CtotNeutralSpecies[i]/x[0] - x[i+1]})
        ...
        for k in range(self.nIter):
           self.Write2Log('==Iter {}==\n'.format(k+1))
           print('==Iter {}==\n'.format(k+1))
           sol = minimize(self.Obj, x0, args = (RPA,self.CtotSpecies),method='SLSQP', bounds=bounds,
                          options={'ftol': self.LSQftol, 'disp': True})
 
           #update solution           
           try:
               #errs = self.Obj(sol.x,RPA,self.CtotSpecies)
               LSQ = sol.fun #np.sum(np.array(errs)**2)
        ...
        gibbsErr = self.GetErr(x_final,RPA,self.CtotSpecies)
        self.Write2Log('\n===errors===\n')
        s = '# dP '
        ...
        self.Write2Log(obs+'\n')
        return fI,CIs,fII,CIIs,gibbsErr, PI, muIs, PII, muIIs, sol.fun
