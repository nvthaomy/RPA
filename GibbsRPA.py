...
class Gibbs_System():
    ...
    def __init__(self,_Program,_Nspecies=1):
        ...
        self.b = [] # RMS length of bond, can be anything for small molecules
        self.V = 10. # box volume
        self.kmin = 0.001 
        self.kmax = 1000 
    ...
    def TakeGibbsStep(self):
        ...
        if self.Converged == False:
            ...
            if self.Program == 'polyFTS' and self.JobType != 'MF' and self.JobType != 'RPA' and self.Barostat != True:
                ...
                mu1 = []
                for i in range(self.Nspecies):
                    C1.append(VarModel1[0]*VarModel1[i+1])
                ...
                mu2 = []
                for i in range(self.Nspecies):
                    C2.append(VarModel2[0]*VarModel2[i+1])

                RPA2 = RPA.RPA(self.Nspecies,self.Nspecies)
                RPA2.Setchain(self.chain)
                RPA2.SetDOP(self.SpeciesDOP)
                RPA2.Setcharge(self.Charges)
                RPA2.SetC(C2)
                RPA2.Setabead(self.abead)
                RPA2.Setu0(self.u0)
                RPA2.SetlB(self.lB)
                RPA2.Setb(self.b)
                RPA2.SetV(self.V)
                ...
                for i in range(self.Nspecies):
                    mu2 = RPA2.mu(i)
                    Operator_List2.append([mu2,0.])
                    
                Operator_List = [Operator_List1,Operator_List2]                 
                self.UpdateParameters(Operator_List)
                self.Iteration += 1
                self.WriteStats()                

                
            elif self.Program == 'polyFTS' and self.JobType != 'MF' and self.JobType != 'RPA' and self.Barostat == True:
                # using a barostat
                JobID_List = []
                RunPaths = []
                p_status_list = []
